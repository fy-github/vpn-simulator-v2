...
class VendorCLIService:
    ...
    def execute_command(
        self, vendor: VendorType, command: str, params: dict[str, Any] | None = None
    ) -> CommandResult:
        """执行厂商命令。"""
        commands = self._cisco_commands if vendor == VendorType.CISCO else self._huawei_commands
        ...
        if not mapping:
            result = CommandResult(
                command=command, output=f"% Unknown command: {command}", success=False
            )
        ...
        result = CommandResult(command=command, output=output, success=True)
        self._history.append(result)

        logger.info(
            "command_executed", vendor=vendor.value, command=command, action=mapping.internal_action
        )
...
# 全局实例
_vendor_cli_service: VendorCLIService | None = None
