"""Integration tests: OpenConnect 真实 TLS + CSTP 握手端到端。"""
...
@pytest.mark.asyncio
async def test_openconnect_tls_cstp_handshake_end_to_end() -> None:
    ...
    try:
        ...
        server_sm = OpenConnectStateMachine()
        client_hs = OpenConnectHandshake(client_reader, client_writer)
        server_hs = OpenConnectHandshake(server_reader, server_writer, state_machine=server_sm)
